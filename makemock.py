...
mock = image_model_eval(truex, truey, truef, trueback, imsz, nc, cff)
mock[mock < 1] = 1. # maybe some negative pixels
variance = mock / gain
mock += (np.sqrt(variance) * np.random.normal(size=(imsz[1],imsz[0]))).astype(np.float32)

f = open('Data/mock_pix.txt', 'w')
f.write('%1d\t%1d\t1\n0.\t%0.3f' % (imsz[0], imsz[1], gain))
f.close()

np.savetxt('Data/mock_cts.txt', mock)

np.savetxt('Data/mock_psf.txt', psf, header='%1d\t%1d' % (nc, nbin), comments='')

truth = np.array([truex, truey, truef]).T
np.savetxt('Data/mock_tru.txt', truth)
